function NWinteractiveCSI3d(im,axppm,im2,axppm2,title)
...
if nargin<2 || isempty(axppm)
    axppm = 1:si(end);
end

if nargin<3, im2 = []; end
if nargin<4, axppm2 = []; end
...
hIm = imagesc('Parent',ax,'cdata',squeeze(real(im(:,:,slice,frame)))); 
...
if isempty(im2)
    hplot = plot(ax2,axppm,abs(squeeze(im(pix(1),pix(2),slice,:))));
else
    hplot = plot(ax2,axppm,abs(squeeze(im(pix(1),pix(2),slice,:))),'-',axppm2,abs(squeeze(im2(pix(1),pix(2),slice,:))),'.-');
end
xlabel('ppm')
axis tight
...
updatePlot
    ...
    function updateImage
        if strcmpi(get(get(bg,'SelectedObject'),'String'),'real')
            set(hIm,'cdata',squeeze(real(im(:,:,slice,frame))))
        elseif strcmpi(get(get(bg,'SelectedObject'),'String'),'imag')
            set(hIm,'cdata',squeeze(imag(im(:,:,slice,frame))))
        elseif strcmpi(get(get(bg,'SelectedObject'),'String'),'phase')
            set(hIm,'cdata',squeeze(angle(im(:,:,slice,frame))))
        elseif strcmpi(get(get(bg,'SelectedObject'),'String'),'mag')
            set(hIm,'cdata',squeeze(abs(im(:,:,slice,frame))))
        else
    ...
    function updatePlot
        if strcmpi(get(get(bg,'SelectedObject'),'String'),'real')
            set(hplot(1),'ydata',squeeze(real(im(pix(2),pix(1),slice,:))))
        elseif strcmpi(get(get(bg,'SelectedObject'),'String'),'imag')
            set(hplot(1),'ydata',squeeze(imag(im(pix(2),pix(1),slice,:))))
        elseif strcmpi(get(get(bg,'SelectedObject'),'String'),'phase')
            set(hplot(1),'ydata',squeeze(angle(im(pix(2),pix(1),slice,:))))
        elseif strcmpi(get(get(bg,'SelectedObject'),'String'),'mag')
            set(hplot(1),'ydata',squeeze(abs(im(pix(2),pix(1),slice,:))))
        else
            warning('unknown radio button')
            set(hplot(1),'ydata',squeeze(abs(im(pix(2),pix(1),slice,:))))
        end
        if ~isempty(im2)
            if strcmpi(get(get(bg,'SelectedObject'),'String'),'real')
                set(hplot(2),'ydata',squeeze(real(im2(pix(2),pix(1),slice,:))))
            elseif strcmpi(get(get(bg,'SelectedObject'),'String'),'imag')
                set(hplot(2),'ydata',squeeze(imag(im2(pix(2),pix(1),slice,:))))
            elseif strcmpi(get(get(bg,'SelectedObject'),'String'),'phase')
                set(hplot(2),'ydata',squeeze(angle(im2(pix(2),pix(1),slice,:))))
            elseif strcmpi(get(get(bg,'SelectedObject'),'String'),'mag')
                set(hplot(2),'ydata',squeeze(abs(im2(pix(2),pix(1),slice,:))))
            else
                warning('unknown radio button')
                set(hplot(1),'ydata',squeeze(abs(im2(pix(2),pix(1),slice,:))))
            end
        end
        xlim(ax2,xl);
    end
